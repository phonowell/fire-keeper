...
const a = async () => {
  ...
  if (!await $.isExisted(`${target}/test.txt`)) throw new Error('0')
}
...
const c = async () => {
  ...
  if (!await $.isExisted(`${temp}/target/test.txt`)) throw new Error('0')
}
...
const e = async () => {
  ...
  if (!await $.isExisted(`${base}/target/test.txt`)) throw new Error('0')
...
const g = async () => {
  ...
  await $.write(`${temp}/test.txt`, 'a little message')
  await $.move(`${temp}/test.txt`, `${temp}/a`, 'b.txt')

  if (!await $.isExisted(`${temp}/a/b.txt`)) throw new Error('0')
}
