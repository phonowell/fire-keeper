...
```typescript
const results = await runConcurrent(2, [() => Promise.resolve(1), () => Promise.resolve(2)])
```
...
**watch**(source, callback, { debounce }): 监听变化（支持 glob，启动时解析一次），返回 unwatch 函数
